	...
	for _, m := range msgs {
		...
		for _, attr := range attrs {
			...
			case nl.TCA_OPTIONS:
				...
				case "dhcmatchall":
					detailed, err = parseDhcMatchAllData(filter, data)
					if err != nil {
						return nil, err
					}
				case "flower":
...
func parseDhcMatchAllData(filter Filter, data []syscall.NetlinkRouteAttr) (bool, error) {
	dhcmatchall := filter.(*DhcMatchAll)
	detailed := true
	for _, datum := range data {
		switch datum.Attr.Type {
		case nl.TCA_MATCHALL_CLASSID:
			dhcmatchall.ClassId = native.Uint32(datum.Value[0:4])
		case nl.TCA_MATCHALL_ACT:
			tables, err := nl.ParseRouteAttr(datum.Value)
			if err != nil {
				return detailed, err
			}
			dhcmatchall.Actions, err = parseActions(tables)
			if err != nil {
				return detailed, err
			}
		}
